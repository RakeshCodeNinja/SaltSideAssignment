 ...
 */
open class BaseNavigator: NavigatorCustom {
  ...
  public required init(with route: Router) {
 ...
 */
public protocol NavigatorCustom: class {
  /// The root navigation controller of your stack.
    typealias BaseNavigatorCompletion = () -> Void
   ...
   */
  func navigate(
    to route: Router, with transition: TransitionType,
  ...
  /**
   ...
   - completion: Completion handler.
   */
  func navigate(to router: NavigatorCustom, animated: Bool, completion: (() -> Void)?)
  ...
  /**
   ...
   - animated: Animate the transition or not.
   */
  func popTo(route: Router, animated: Bool)
...
public extension NavigatorCustom {
  func navigate(
    to route: Router, with transition: TransitionType,
    animated: Bool = true, completion: (() -> Void)? = nil
  ) {
    let viewController: UIViewController = route.screen
    switch transition {
    case .modallyOverCurrentContext:
    ...
    case .push:
//        if let vController = rootViewController?.viewControllers.first(where: { $0.typeName == viewController.typeName }) {
//            route.transitionConfigurator?(currentViewController, vController)
//            rootViewController?.popToViewController(vController, animated: true)
//        } else {
            route.transitionConfigurator?(currentViewController, viewController)
...
//        }
  ...
  func popToRoot(animated: Bool = true, _ completion: BaseNavigatorCompletion? = nil) {
  ...
  func popTo(route: Router, animated: Bool = true) {
    guard
      let viewControllers = rootViewController?.viewControllers,
